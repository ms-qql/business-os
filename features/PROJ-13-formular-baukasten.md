...
## QA Test Results
**Datum:** 2026-08-23 · **Ergebnis: NOT READY** — 3 High-Bugs

### Automatisierte Tests
- Backend `pytest`: 209 passed, 13 failed. Alle 13 Fehler vorbestehend (xhtml2pdf-Lücke in `angebote`/`rechnungen`, unabhängig von PROJ-13) — verifiziert per eigenem Testlauf, deckt sich mit Backend-Bericht.
- `tests/features/formulare/test_formulare.py`: 4/4 grün (Builder-Flow+Publish, öffentliche atomare Übernahme, Spam-Markierung).
- Frontend `npx tsc --noEmit`: sauber, keine neuen Fehler.

### Acceptance Criteria (gegen Code + eigene Testläufe)
1. Formulare anlegen/bearbeiten/veröffentlichen für Inhaber UND Büro, fremde Mandanten unsichtbar — **FAIL**: siehe BUG-1. Mandanten-Isolation selbst geprüft (Cross-Tenant GET → 404, falsche Domain → 404) — PASS.
2. Fester Feldtypen-Katalog (10 Typen) — PASS (schemas.py `FeldTyp` Literal, DB-CHECK-Constraint identisch).
3. Pflichtfelder Bezeichnung/Pflicht/Hilfetext, geordnete Optionen — PASS (Code gelesen, `FeldWrite`/`formular_option`).
4. Beliebig viele Schritte, Fortschritt, Vor/Zurück mit Werterhalt — strukturell PASS (Code gelesen, kein Live-Browser-Test möglich ohne laufenden Stack).
5. Komplexitätsstufen Einfach/Erweitert — PASS (`optional_in_einfach`, Serverfilter in `_validate_werte`).
6. Direktlink/iframe/JS-Snippet, nur veröffentlichte Fassung — PASS (`get_einbindung`).
7. SHK-/Entrümpelungs-Vorlage, danach anpassbar — PASS (`_apply_vorlage`, Vorlage nur beim Anlegen kopiert).
8. Servervalidierung Pflicht/Datum/Zahl/Auswahl/Consent, deutsche Fehler — PASS (`_validate_werte`, eigener curl-Testlauf mit fehlendem Consent nicht separat nötig, Code + Testfall decken es ab).
9. Upload JPEG/PNG/WebP/PDF mit Größenlimit, deutsche Fehler bei Verstoß — PASS (`_sniff_upload` Magic-Bytes, 15 MB Limit).
10. Gültige Einsendung erzeugt genau eine Anfrage + Kundenentwurf, kein Projekt — PASS (eigener Testlauf `test_public_submit_atomic_takeover` bestätigt Kunde+Anfrage, keine Vorgangs-Projekt-Verknüpfung außerhalb Spec).
11. E-Mail-Treffer ordnet bestehendem Kunden zu, sonst Kundenentwurf — PASS (`get_kunde_by_email`-Pfad, Code gelesen).
12. Honeypot/Zeitvalidierung/Rate-Limit, Spam bleibt nur für Berechtigte nachvollziehbar — **FAIL**: siehe BUG-2 (Honeypot/Zeitfenster funktionieren und sind per Testlauf bestätigt; aber "nur für berechtigte Nutzer nachvollziehbar" ist nicht erfüllbar, da kein Leseweg existiert).
13. Endpunkt nicht erreichbar → keine Anfrage, wiederholbarer Fehler — strukturell PASS (Frontend-Code zeigt Retry-Screen, Backend-Transaktion atomar).
14. Formularänderung bleibt Entwurf bis Publish, Einbindung zeigt weiter alte Fassung — PASS (unveränderlicher `formular_version`-Snapshot, `withdraw` trennt nur Referenz).
15. Deutsche Texte, Tastaturbedienbarkeit, ab 375px ohne horizontales Scrollen — strukturell PASS (Code-Review UI-Komponenten, kein Live-Browser-Test durchgeführt, siehe Limitation).

**Ergebnis: 12/15 strukturell/funktional PASS, 3 FAIL (siehe Bugs). Kein Live-Browser-E2E durchgeführt (Backend-Stack in diesem QA-Lauf nicht gestartet); Bewertung basiert auf eigenem pytest-Lauf + Code-Verifikation + gezielten Red-Team-pytest-Fällen.**

### Gefundene Bugs

**BUG-1 (High) — Büro-Rolle kann keine Formulare verwalten, verstößt gegen AC1 + Tech Design**
`backend/app/features/formulare/routes.py:20`: `_owner = require_role("Inhaber")` wird auf ALLE `/formulare/*`-Schreibrouten angewendet. Tech Design (`API-Contracts`, Zeile 161) und AC1 verlangen explizit `require_role("Inhaber", "Buero")` — analog zum etablierten Muster in `kunden/routes.py:11`, `vorgaenge/routes.py:12`. Eigener Testlauf bestätigt: Büro-Nutzer erhält `403 {"detail":"Erforderliche Rolle nicht vorhanden."}` bei `POST /formulare`.
Fix: `_owner = require_role("Inhaber", "Buero")` in `routes.py`.

**BUG-2 (High) — Spam-Liste nicht erreichbar (`GET /formular-einsendungen?spam=1` fehlt komplett)**
Tech Design (API-Contracts, Zeile 202) und AC12 fordern einen Lesepfad `GET /formular-einsendungen?spam=1&limit=&offset=` für Inhaber/Büro. Der Service (`public_service.list_spam_einsendungen`) und Repository (`repository.list_einsendungen_spam`) existieren, sind aber nirgends verdrahtet — kein Router registriert diese Funktion (`grep` über `routes.py`/`main.py` ohne Treffer). Als markiert erkannte Spam-Einsendungen sind für niemanden im System einsehbar; verstößt gegen "bleiben nur für berechtigte Nutzer nachvollziehbar" (AC12) sowie gegen die per Architecture-Review Runde 1 explizit geforderte Pagination-Route.
Fix: neue Route (z. B. in `routes.py`) für `GET /formular-einsendungen` mit `require_role("Inhaber","Buero")`, verdrahtet auf `public_service.list_spam_einsendungen` + `FormularEinsendungListeResponse`.

**BUG-3 (High) — Formularantwort fehlt in `GET /vorgaenge/{id}`**
Tech Design (Datenmodell-Tabelle `formular_einsendung`, Zeile 136) und API-Contracts (Zeile 205–208) sowie die im Architecture-Review Runde 2 explizit getroffene Produktentscheidung verlangen, dass `GET /vorgaenge/{id}` um den unveränderlichen Formularantwort-Snapshot ergänzt wird (Werte, Consent, Anhänge), sichtbar für Inhaber/Büro und zugewiesene Monteure. `backend/app/features/vorgaenge/schemas.py` (`VorgangDetail`) und `service.py::get_vorgang_detail` enthalten keinerlei Formular-/Einsendung-Feld; `grep -r formular backend/app/features/vorgaenge/` liefert keinen Treffer. Büro-Mitarbeiter können eine aus Formular übernommene Anfrage im Vorgang nicht als solche erkennen oder die eingereichten Werte/Anhänge einsehen — Kernnutzen des Features ("gültige Einsendung als neue Anfrage mit Kundenentwurf vorfinden, ohne Übertragen weiterbearbeiten") ist nicht erfüllt.
Fix: `VorgangDetail` um optionales `formular_einsendung`-Feld erweitern; `get_vorgang_detail` lädt bei vorhandener `anfrage.formular_einsendung_id` den Snapshot aus `formular_einsendung` (Werte, Consent, verknüpfte Uploads/Dokumente) und hängt ihn an — Rollenprüfung bleibt die bestehende `_guard_monteur_read`.

### Security-Red-Team (eigene Testläufe)
- Cross-Tenant `GET /formulare/{id}` mit JWT von Mandant B auf Formular von Mandant A → 404 (kein Leak). **PASS**
- Öffentlicher Snapshot über falsche/fremde Betriebsdomain → 404, kein Formular-/Mandanteninfo-Leak. **PASS**
- RLS-Policies auf allen 7 neuen Tabellen vorhanden (`FOR ALL USING (mandant_id = current_setting(...))`), Migration `011_formular_baukasten.sql` verifiziert.
- Honeypot + Zeitfenster-Spam-Erkennung funktional (eigener Testlauf `test_public_submit_spam` reproduziert und bestanden).
- Mandant wird ausschließlich hostbasiert aufgelöst; `x-forwarded-host` nur bei verifiziertem internem Proxy-Secret vertraut (`_hostname()` in routes.py) — kein Header-Spoofing-Vektor gefunden.
- Kein SQL-Injection-Vektor gefunden: alle Repository-Queries nutzen parametrisierte Platzhalter (`%s`), keine String-Interpolation von Nutzereingaben in SQL.

### Regression
- Vollständige Backend-Suite: 209 passed / 13 failed, alle 13 Fehler vorbestehend (xhtml2pdf, PROJ-5/PROJ-8 betroffen, unabhängig von diesem Feature) — deckt sich exakt mit dem vom Backend-Worker gemeldeten Stand.
- Keine Frontend-Regression: `tsc --noEmit` sauber.

### Limitation
- Kein Live-Browser-E2E (`abc-qa-e2e`) in diesem Lauf: Backend-Server/Flutter- bzw. Next.js-Build wurden für diese QA-Runde nicht gestartet. Responsive- und Tastaturbedienbarkeits-Kriterien (AC15) sind nur strukturell (Code-Review) geprüft, nicht visuell verifiziert.
- Tech-Design-Datei fehlte initial im Worktree (Feature-Branch-Datei nur im Haupt-Repo committed) — für diesen QA-Lauf aus dem Haupt-Repo kopiert, um gegen die tatsächliche Spec zu testen.

### Produktionsreife
**NOT READY** — 3 High-Bugs (BUG-1, BUG-2, BUG-3) müssen vor Deploy gefixt werden.
