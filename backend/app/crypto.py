...
def _fernet() -> Fernet:
    """Initialise only when a mailbox secret is actually used.

    A missing deployment key must not prevent authentication or the rest of
    the application from starting.
    """
    return Fernet(settings.email_credentials_key)
